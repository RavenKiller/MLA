...
class MLANet(Net):
    ...
    def __init__(
        ...
        if model_config.INSTRUCTION_ENCODER.bidirectional:
            self.low_inst_size = (
                2 * model_config.INSTRUCTION_ENCODER.hidden_size
            )
        else:
        ...
        self.v_size = (
            model_config.CLIP.feature_size
            + model_config.DEPTH_ENCODER.feature_size
        )
        ...
        self.final_input_compress = nn.Sequential(
            nn.Linear(
                self.all_feature_size, model_config.STATE_ENCODER.hidden_size
            ),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(
        ...
        depth_embedding = self.depth_encoder(observations).flatten(
            2
        )  # (N,D,L)
        ...
        instruction_embedding = instruction_embedding.permute(
            0, 2, 1
        )  # (N,L,D)
