...
class IWTrajectoryDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        self.preload_size = batch_size * 22
        self._preload = []
    ...
    def _update_dataset(self, data_it):
        ...
        with tqdm.tqdm(
            ...
            while collected_eps < self.config.IL.DAGGER.update_size:
                ...
                # model, preprocessor = clip.load("ViT-B/32", device=self.device)
                # ground_image = Image.fromarray(origin_observations[0]["rgb"])
                # ground_image = preprocessor(ground_image).unsqueeze(0).to(self.device)

                # text = clip.tokenize([origin_observations[0]["instruction"]["text"]]).to(self.device)
                # ground_text_features = model.encode_text(text)
                # ground_image_features = model.encode_image(ground_image).float()

                # # batch_ob_error = (batch["rgb"]-cc).abs().sum()
                # image_error_res = (ground_image-actual_image).abs().sum()

                # text_error_res = (text_features-ground_text_features).abs().sum()
                # rgb_error_res = (rgb_features[0]-ground_image_features).abs().sum()

                for i in range(envs.num_envs):
                    if rgb_features is not None:
                        observations[i]["rgb_features"] = rgb_features[i]
                        observations[i]["rgb_seq_features"] = rgb_seq_features[
                            i
                        ]
                ...
                for epoch in tqdm.trange(
                    ...
                    for batch in batch_bar:
                        ...
                        batch_bar.set_postfix(
                            {
                                "loss": "%2.4f" % (loss),
                                "ld": self.policy.net.peak_loss_lambda,
                            }
                        )
                        writer.add_scalar(
